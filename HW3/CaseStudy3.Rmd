...
```{r}
counts <- matrix(data = NA, nrow = B, ncol = 2)
for (i in 1:B) {
  indexHighestCount <- 0
  tempFirstIndex <- 0
...
## THE REST IS DEMONSTRATIVE THAT THIS WORKS AT THE INITIAL SEED
#summary(counts)
threePalindromes <- which(counts[,1] == 3)
threePalindromes
monteCarloSampleDists[814, 215:218]
counts[814,]
```
